...
function setup() {
  ...
  trex = createSprite(camera.position.x+50,camera.position.y+120,20,50);
  trex.addAnimation("running", trex_running);
  trex.scale = 0.5;
  
  ground = createSprite(camera.position.x,camera.position.y+180,400,20);
  ground.addImage("ground",groundImage);
  //ground.x = ground.width /2;
  //ground.velocityX = -2;
  
  invisibleGround = createSprite(camera.position.x,camera.position.y+190,400,10);
...
function spawnObstacles() {
  if(World.frameCount % 60 === 0) {
    var obstacle = createSprite(camera.position.x+260,165,10,40);
